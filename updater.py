...
REPOSITORY = "feiwu6733-arch/linsi-edit-lite"
...
def allowed_path(name):
    ...
    if name.startswith("/") or any(p.lower() in (".", "..", ".local", ".venv", ".git", "artifacts", "__pycache__") or p.endswith((" ", ".")) or re.fullmatch(r'(CON|PRN|AUX|NUL|COM[0-9]|LPT[0-9])',p.upper().split('.')[0]) for p in parts):
        return False
...
def inspect_package(package, expected_version, expected_digest):
    ...
    with zipfile.ZipFile(package) as archive:
        ...
        if manifest.get("product") != "Linsi Edit Lite" or manifest.get("version") != expected_version or manifest.get("update_format") != 1:
        ...
        if not {'serve.py', 'package.json', 'start.ps1', 'release-files.json'} <= set(listing):
...
class Updates:
    ...
    def check(self):
        try:
            ...
            asset=next((a for a in release.get('assets',[]) if a.get('name')==f'linsi-edit-lite-{latest}.zip'),None)
            if asset:
...
def run_install(root, data_dir, port, pid):
    ...
    def launch():
        ...
        try:return subprocess.Popen([sys.executable,str(root/'serve.py'),'--port',str(port),'--data-dir',str(data_dir),'--no-browser'],cwd=root,creationflags=flags,stdout=log,stderr=log)
        finally:log.close()
    try:
        wait_process(pid)
        ...
        for _ in range(60):
            ...
            try:
                ...
                if health.get('product')=='linsi-edit-lite' and health.get('version')==target_version and health.get('pid')==child.pid:healthy=True;break
